...
export default async function WorkingHolidayVisaIndexPage() {
	const items = await fetchWorkingHolidayVisas()
	return (
		<main className="mx-auto w-full max-w-[1200px] px-4 sm:px-6 lg:px-8 py-12">
			<div className="text-center">
				<div className="text-[#2dc0d9] font-semibold tracking-wide text-sm">WORKING HOLIDAY VISA</div>
				<h1 className="mt-2 text-3xl sm:text-5xl font-semibold text-[#283277]">Choose Your Working Holiday Visa</h1>
			</div>
			{items.length === 0 ? (
				<p className="mt-10 text-center text-sm text-neutral-600">No working holiday visa items found.</p>
			) : (
				<div className="mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-center">
					{items.map((it) => {
						const acf: any = (it as any)?.acf || {}
						const title = (acf?.service_name as string) || it.title?.rendered || "Working Holiday Visa"
						const subheading = (acf?.service_subheading as string) || ""
						const img = (acf?.service_image?.url as string) || undefined
						return (
							<Link href={`/working-holiday-visa/${it.slug}`} key={it.id} className="w-full max-w-[360px] mx-auto rounded-lg overflow-hidden flex flex-col relative group cursor-pointer hover:scale-105 transition-transform duration-300">
