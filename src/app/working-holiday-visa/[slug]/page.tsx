...
export default async function WorkingHolidayVisaDetail({ params }: { params: { slug: string } }) {
	const [item, all] = await Promise.all([
		fetchWorkingHolidayVisaBySlug(params.slug),
		fetchWorkingHolidayVisas(),
	])
	if (!item) return notFound()
	const acf: any = (item as any)?.acf || {}

	const title = (acf?.service_name as string) || item.title?.rendered || "Working Holiday Visa"
	...
	const sec3Subheading = (acf?.["3rd_section_subheading"] as string) || ""
						...
						<nav className="rounded-xl bg-white ring-1 ring-black/5 overflow-hidden">
							<ul className="divide-y divide-black/5">
								{all.map((it) => {
									...
									const label = (iacf?.service_name as string) || it.title?.rendered || "Working Holiday Visa"
									return (
										<li key={it.id}>
											<Link href={`/working-holiday-visa/${it.slug}`} className={`flex items-center justify-between gap-3 px-4 py-4 text-[15px] transition ${isActive ? "bg-[#e94a4a] text-white" : "bg-white hover:bg-neutral-50 text-neutral-800"}`}>
					...
					<article className="md:col-span-8">
						...
						{sec3Heading || sec3Points.length || sec3Image ? (
							<section className="mt-10">
								...
								{sec3Subheading ? (
									<p className="mt-3 text-[15px] leading-7 text-neutral-700">{sec3Subheading}</p>
								) : null}
								{sec3Image ? (
